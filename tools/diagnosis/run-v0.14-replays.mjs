/* v0.14 evidence sweep — Credit Restored gated on Credit ≥ 6. */
import { runDiagnosisBatch } from './sim-v0.14.mjs';
import { writeFileSync, mkdirSync } from 'node:fs';

const OUT_DIR = 'E:/AI/sovereign/experiments/v0.14-failure-pressure-candidate/raw-data';
...
let creditRestoredFires = 0, creditRestoredUngated = 0, creditRestoredGated = 0;
...
for (const g of c400.games) {
  ...
  const crUngatedFires = creditEvs.filter(e => e.reason === 'Credit Restored');
  ...
  /* Credit Restored ungated count = appears in pressureEvents */
  creditRestoredUngated += crUngatedFires.length;
  /* Gated count = ledger NO EFFECT row for Credit Restored — we don't capture
   * the full ledger in per-game telemetry, but we can detect by counting actual
   * Credit Restored card draws. Each draw either fires +1 (ungated, in pressure
   * events) or fires NO EFFECT (gated, not in pressure events).
   * We don't have the full ledger here. To estimate gated fires, we use a
   * proxy: each game's bankruptcyTriggerReasons might mention it via cash
   * events, but that's not reliable. Best: skip per-game gated count;
   * recovery-rate metric below is what matters.
   */
  ...
  /* Recovery analysis: did credit dip to 6 (or 5) and later go to 7+? */
...
console.log(`  Credit Restored ungated (+1 fired): ${creditRestoredUngated}`);
...
console.log(`  → Post-Funding ≤ 5 (reach 5): ${pfmAt5}  (v0.13: 3)`);
console.log(`  → Post-Funding < 5 (below 5): ${pfmBelow5}  (v0.13: 0)`);
...
console.log(`    Recovery rate: ${(recoveredFrom6/dipTo6*100).toFixed(1)} %  (v0.13: 34 %; target < 20 %)`);
