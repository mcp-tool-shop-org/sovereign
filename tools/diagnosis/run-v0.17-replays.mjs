/* v0.17 evidence sweep — Speculation Fever fragile-credit escalation.
 * Conditional: Credit -1 if Credit ≥ 7, Credit -2 if Credit ≤ 6.
 * Layered on v0.16 base (Anti-Federalist Pamphlet Credit -1).
 * Bank Run v0.11 preserved. NO recovery gates.
 */
import { runDiagnosisBatch } from './sim-v0.17.mjs';
import { writeFileSync, mkdirSync } from 'node:fs';

const OUT_DIR = 'E:/AI/sovereign/experiments/v0.17-failure-pressure-candidate/raw-data';
...
/* Headline + pressure-overlap + Spec Fever delta split + floor distribution + recovery analysis */
...
let dipTo5OrBelow = 0, recoveredFrom5OrBelow = 0;
let dipBelow5 = 0, recoveredFromBelow5 = 0;
const floorDist = { 6: 0, 5: 0, 4: 0, 3: 0, 2: 0, 1: 0, 0: 0 };
const recoverySrc = {};
const resistMaxDist = {};
...
for (const g of c400.games) {
  ...
  if (pfm <= 6) floorDist[Math.max(0, pfm)] += 1;
  ...
  if (firstFiveOrBelowIdx >= 0) {
    ...
    if (recovered) recoveredFrom5OrBelow += 1;
  }
  if (firstBelowFiveIdx >= 0) {
    ...
    if (recovered) recoveredFromBelow5 += 1;
  }
...
console.log(`    SF -2 (Credit ≤ 6):     ${sfMinus2Fires}  (Claude Design advisory: 5)`);
...
console.log(`  → Post-Funding ≤ 5 (reach 5): ${pfmAt5}  (v0.16: 6; target > 6)`);
console.log(`  → Post-Funding < 5 (below 5): ${pfmBelow5}  (v0.16: 0; target > 0)`);

console.log('\nCredit FLOOR distribution (games where credit min ≤ k):');
for (const k of [6, 5, 4, 3, 2, 1, 0]) {
  const sum = pfmKeys.filter(x => x <= k).reduce((a, x) => a + postFundingMinDist[x], 0);
  console.log(`  ≤ ${k}: ${sum} games  (this floor: ${floorDist[k]})`);
}

console.log('\n=== RECOVERY ANALYSIS ===');
console.log(`  Games where Credit dipped to 6: ${dipTo6}`);
console.log(`    Recovered to 7+: ${recoveredFrom6}`);
console.log(`    Recovery rate: ${(recoveredFrom6/dipTo6*100).toFixed(1)} %  (v0.16: 35.7 %)`);
console.log(`  Games where Credit dipped to ≤ 5: ${dipTo5OrBelow}`);
console.log(`    Recovered to 7+: ${recoveredFrom5OrBelow}`);
console.log(`    Did NOT recover: ${dipTo5OrBelow - recoveredFrom5OrBelow}`);
console.log(`  Games where Credit dipped BELOW 5: ${dipBelow5}  (v0.16: 0)`);
console.log(`    Recovered to 7+: ${recoveredFromBelow5}`);
console.log(`    Did NOT recover: ${dipBelow5 - recoveredFromBelow5}`);
...
console.log('\n=== TIMING-LIMIT CLASSIFICATION ===');
if (sfMinus2Fires < 5) {
  console.log(`  Spec Fever -2 fires: ${sfMinus2Fires} / ${N}  → TIMING-LIMITED (< 5)`);
  console.log(`  Next lever recommendation: fourth independent Credit-down source (not another conditional escalation)`);
} else {
  console.log(`  Spec Fever -2 fires: ${sfMinus2Fires} / ${N}  → at/above threshold`);
}
