...
> **Status — v1.4.0 (beta).** First substantive feature release since v1.1.2 (circuit-victory). Adds three layers on top of the circuit base — **strategic depth** (profile-locked Special Actions, HAND cards with timing windows, Reform recovery, multi-stage Public Credit pressure), **strategic arc** (Federal Era Events firing each round from round 8, three Profile Visions with endgame bonuses), and **the Chronicler** (a named third-person historical voice that surfaces 14 event-bound banners across the game, sourced from a verified historical quote bank — 27 real quotes from Hamilton / Madison / Jefferson / Adams / Gallatin / Maclay / Freneau with traceable founders.archives.gov, Wikisource, and Library of Congress URLs; zero fabricated attributions). Median game length stays ~23 rounds / 67 turns; circuit-triggered Final Accounting unchanged. Three viable profile paths: Treasury 59 % / Merchant 20 % / Manufacturer 21 % (CANONICAL × 100). The printable board game remains stable at v0.2. See `CHANGELOG.md` for the full delta and beta caveats.
...
- **Digital mode** — single self-contained HTML file. Circuit-based end condition: the game ends when one player completes their fourth crossing of Treasury Opens. Median game length ~23 rounds (67 turns). At Final Accounting the highest-Influence player wins, *not necessarily the one who got around the board first*. Strategic depth layer: three profile-locked Special Actions, six HAND cards with timing windows, Reform recovery action, multi-stage Credit pressure (Public Doubt / Crisis / Panic / Default). Strategic arc layer: eight Federal Era Events firing each round from round 8, three Profile Visions with endgame bonuses. Chronicler narration layer: 14 event-bound historical banners, real quotes from the Federalist Papers and Founders Online, persistent toast with × dismiss. Deterministic mulberry32 RNG, scripted AI opponents, save / load with hash integrity, replay scrubber, designer-gated batch simulation tool.
- **Balance baseline** — circuit + strategic-depth + strategic-arc + Chronicler (v1.4.0 beta): Treasury 59 % · Merchant 20 % · Manufacturer 21 % (CANONICAL × 100). All three profiles win meaningfully. Vision achievement rates: Federal Credit Architect 54 %, Commerce Sovereign 39 %, Industrial Founder 29 %. The underlying v0.18 mechanics (Credit Crisis, cash IP scoring, Industrial Charter, set completion bonuses) are preserved byte-identical from the v0.3 → v0.10 → v0.18 design arc driven by 1,000+ deterministic simulation games.
...
- **Strategic depth (v1.2 layer)** — three profile-locked Special Actions (Issue Federal Bond / Broker Route Contract / Charter Workshop), 6 HAND cards with timing windows (hand cap 2), Reform recovery action, multi-stage Credit pressure (Public Doubt / Crisis / Panic / Default).
- **Strategic arc (v1.3 layer)** — 8 Federal Era Events firing each round from round 8+ (5 choice + 3 auto), 3 Profile Visions (Federal Credit Architect / Commerce Sovereign / Industrial Founder) with +3 IP endgame bonus.
...
- **v1.4.0 is a beta.** Each vertical slice (strategic depth, strategic arc, Chronicler) was structurally audited and validated against simulation diagnostics; the full four-layer stack has been cold-walked at slice level but not end-to-end by a fresh human player. Numbers from CANONICAL × 100: 100 / 100 trigger circuit-end, median 23 rounds, 59 / 20 / 21 winner split, ~6-8 Chronicler banners per game, ~33 reactions per game. Treat as opt-in until you've walked it yourself.
- **AI profiles do not yet adapt to v1.2-v1.4 mechanics.** They run v0.18 decision functions — they don't "race for the Vision" or "spend HAND cards strategically" the way a human player would. Real human play will diverge from CANONICAL × 100 measurements.
